...
| **9 — HRIS / ATS** | **Foundation shipped.** 3 new tables (`external_integrations` with Fernet-encrypted creds, `external_id_mappings` with composite uniques, `integration_sync_logs`). `services/integrations/` package with `IntegrationAdapter` ABC, working `MockAdapter` (in-memory store, all push/pull/get_stage_catalog methods implemented), and DOCUMENTED STUBS for `MergeAdapter` / `GreenhouseAdapter` / `LeverAdapter` (TODO comments map to specific endpoints + auth flows). `services/integrations/sync_engine.py` does the full pull cycle: list_jobs/candidates/applications → upsert via ExternalIdMapping → preserves AI/fraud/tag fields under HireOps ownership; logs every run. Worker spawns at app boot alongside mailbox_listener/call_queue/outreach, polls every 15 min. Router: GET `/integrations/hris/available`, GET `/integrations/hris`, POST `/connect/{provider}`, DELETE, POST `/sync`, POST `/toggle`, GET `/status`, GET `/logs`, PUT `/mapping`. Probes credentials on connect; sets sync_status='auth_failed' on bad creds without rolling back the save. UI: `/settings/hris-integrations` with provider catalog cards (Mock enabled, real adapters marked "Coming soon"), connect modal with auth-method-specific fields (seed/api_key/public_token/oauth), per-integration drawer with status, last-error, sync-now/disconnect controls, and a recent-syncs log table. **Real adapters (Merge/Greenhouse/Lever)** and **push hooks** are intentionally deferred — not on the roadmap for now. | Deferred — not on current roadmap. |
