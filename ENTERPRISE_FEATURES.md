...
| **2 — Candidate Tags** | Nothing | Build per spec. Note: profile_skills (auto LLM tags) already exist on Candidate; don't conflate the two — manual user-applied tags are separate. |
...
| **4 — Custom Interview Questions** | Existing `agents/qa_interview.py` and `agents/voice_screener.py` generate questions per round (apt, reasoning, technical) from CV + JD. | Add a `custom_questions` field on `Job` that the agents read first before LLM-generating remaining rounds. |
...
| **7 — Offer Letter + E-Sign** | Nothing | Standalone — build per spec. |
