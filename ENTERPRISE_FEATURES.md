...
| **3 — Custom Hiring Stages** | **Shipped.** 3 new tables (`pipeline_templates`, `pipeline_stages`, `application_stage_transitions`). `Application.stage` (legacy string) is preserved alongside `current_stage_id` (new FK) so every existing read path keeps working. Init-time `_seed_default_pipeline_templates()` auto-seeds a `Default` system template per tenant with the 7 legacy stage keys (`new`, `classified`, `matched`, `screening_scheduled`, `screened`, `shortlisted`, `rejected`) — terminal flags set on `shortlisted` (hired) and `rejected`. Backfills `Job.pipeline_template_id` to the default and `Application.current_stage_id` from the matching key. Routers: full CRUD for templates + stages, reorder, clone, plus extended `PATCH /applications/{id}/stage` that accepts `stage_id` AND/OR legacy `stage` string and writes an `ApplicationStageTransition` row each move. UI: `/settings/pipeline-templates` list + `/settings/pipeline-templates/{id}` editor (chevron-reorder, inline label/key/colour/terminal-outcome/auto-advance threshold edit, system-template name lock). Quick-link card on Settings. Auto-pipeline (`workflow_service`) keeps writing the legacy string — the seeded default template's keys match exactly so the transitions table fills in via the PATCH endpoint over time. | Job edit dropdown to swap templates, list-view stage colour rendering from `current_stage.color`, candidate detail stage dropdown sourced from job's template — all small follow-ups. |
...
| **5 — Recruiter Productivity** | **Shipped.** `events.actioned_by_user_id` migration. `_log_event` helpers thread `actor_user_id` through every HR-driven event (matched, stage_changed, rescored, interview_link_generated, interview_slot_booked, evaluated, final_score_calculated, rejection_email_sent). `services/recruiter_metrics.py` aggregates candidates_added / applications_progressed / interviews_evaluated / offers_extended / hires_made / avg_time_to_screen_hours / conversion ratios per recruiter. Owner-only `GET /api/v1/metrics/recruiters` + `/leaderboard`. UI at `/reports/recruiters` with date picker (7d/30d/90d shortcuts), 4 leaderboard cards, sortable per-column table, CSV export. Shows "Data starts {date}" banner derived from first attributed event. **Per-recruiter LLM cost is now live**: `llm_usage.user_id` column + idempotent migration; auth `current_session` publishes `set_active_user(s.user.id)` alongside `set_active_tenant`; `record_llm_usage()` and the `LLMCallTimer` exit hook persist `user_id` against the `_active_user` contextvar; `recruiters_summary()` sums `LlmUsage.cost_usd WHERE user_id = ?` per recruiter and surfaces it as `llm_cost_usd` on the leaderboard. Background workers (mailbox_listener) deliberately leave `user_id` NULL so auto-pipeline cost isn't attributed to any individual. | — |
| **6 — Sequenced Outreach** | **Shipped.** 4 new tables (outreach_sequences, outreach_steps, outreach_enrollments, outreach_messages). New `services/outreach_worker.py` polls every 60s and dispatches due messages via existing smtp_service / twilio_service (added send_sms there). Reply detection wired into `mailbox_listener` — inbound email from a candidate stops their active enrollments when sequence.stop_on_reply=True. Merge tags resolved at send time: `{{candidate.first_name}}`, `{{candidate.email}}`, `{{job.title}}`, `{{recruiter.name}}`, `{{tenant.name}}`. Each message also writes a Communication row so the candidate timeline reflects every send. Routers: full sequences/steps/enrollments CRUD with reorder + bulk-enroll + stop/pause/resume. UI: `/outreach` list with reply-rate stats per sequence, `/outreach/{id}` editor with chevron-reorder steps + inline edit + right-rail enrollment list, "Enroll in sequence" modal on the Talent Bank's selection bar. | — |
...
| **8 — Pipeline Forecasting** | **Shipped.** New `pipeline_forecasts` table caches the latest run per (tenant, job, window). `services/forecast_service.py` computes per-stage conversion rates + median time-in-stage from the last 90 days of `application_stage_transitions`, walks each open application's path to the hired terminal, multiplies prob_reach × prob_within_window (exp-decay past the window), bootstrap-samples 1000 trials for a 90% confidence band. Cold-start: tenants with no transition history get a 30% default per-stage rate + 48h default time-in-stage (documented in code). Cache TTL 6h; force-recompute available. API: GET `/forecasts/pipeline?job_id=&window_days=` + POST `/recompute`. UI: `PipelineForecastCard` component drops onto the dashboard ("Hiring forecast") and per-job detail page, with 30/60/90d window switcher, per-application contribution histogram strip, cold-start banner, and a recompute button. | — |
| **9 — HRIS / ATS** | **Foundation + Merge.dev + push hooks shipped.** 3 new tables (`external_integrations` with Fernet-encrypted creds, `external_id_mappings` with composite uniques, `integration_sync_logs`). `services/integrations/` package with `IntegrationAdapter` ABC, working `MockAdapter`, and a fully wired-up **`MergeAdapter`** using `httpx` against `api.merge.dev/api/ats/v1`: `test_connection` (GET /account-details), paginated `list_jobs / list_candidates / list_applications` (cursor pagination, bounded by `MERGE_MAX_PAGES`), `push_candidate` (POST /candidates with email/phone arrays), `push_stage_change` (passthrough PATCH /applications/{id}), `push_hire` (routes through the stage-change path). Lazy `public_token → account_token` exchange on first call. Registry sets `enabled` dynamically based on whether `MERGE_API_KEY` is in env — without it, the UI greys the connect card. `GreenhouseAdapter` / `LeverAdapter` remain documented stubs. **Push hooks (`services/integrations/push_hooks.py`)**: best-effort fire-and-forget propagation to every connected provider. `push_application_created` from `workflow_service` after each successful application save (skipped when fraud-blocked). `push_stage_changed` + conditional `push_hire` from `applications.update_stage`, `applications.bulk_update_stage`, AND the threshold auto-advance/reject path in `screening.py` — covering both manual and AI-driven stage transitions. Every push writes an `IntegrationSyncLog` row with `direction='push'`; new external ids land in `ExternalIdMapping` for idempotency. `services/integrations/sync_engine.py` does the full pull cycle: list_jobs/candidates/applications → upsert via ExternalIdMapping → preserves AI/fraud/tag fields under HireOps ownership. Worker spawns at app boot alongside mailbox_listener/call_queue/outreach, polls every 15 min. Router: GET `/integrations/hris/available`, GET `/integrations/hris`, POST `/connect/{provider}`, DELETE, POST `/sync`, POST `/toggle`, GET `/status`, GET `/logs`, PUT `/mapping`. UI: `/settings/hris-integrations` with provider catalog cards, connect modal, per-integration drawer with status / last-error / sync-now / disconnect / recent-syncs log. | Native Greenhouse + Lever adapter network code (Merge.dev is now the recommended path for breadth). |
