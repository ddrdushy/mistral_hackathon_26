      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-lg bg-violet-50 text-violet-600 flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h2m-16 0h2M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <div className="min-w-0">
              <div className="text-sm font-semibold text-slate-900 flex items-center gap-2">
                Organization Profile
                {me && !me.tenant.profile_completed && (
              ...
              <div className="text-xs text-slate-500 mt-0.5">
                Industry, headquarters, currency — grounds AI features in your
                real company instead of generic placeholders.
              </div>
            </div>
          </div>
        </Link>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold text-slate-900">Calendar Integration</div>
              <div className="text-xs text-slate-500 mt-0.5">
                Connect your Google Calendar so interview slot suggestions skip times you&apos;re already booked.
              </div>
            </div>
          </div>
        </Link>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold text-slate-900">Job Boards</div>
              <div className="text-xs text-slate-500 mt-0.5">
                One-click publish to LinkedIn, Indeed, Facebook, MyFutureJobs.
              </div>
            </div>
          </div>
        </Link>
  ...
  const backfill = async () => {
    ...
    try {
      const res = await apiPost<{
        result: { scanned: number; added: number; skipped: number };
        tenant: VoiceTenantSummary;
      }>("/settings/voice/backfill");
      setBackfillMsg(
        `Scanned ${res.result.scanned} interviews · added ${res.result.added} new usage row${res.result.added === 1 ? "" : "s"} · ${res.result.skipped} already recorded.`,
      );
      await load();
    } catch (err) {
      setBackfillMsg(
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5 mb-6">
      <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
        ...
        <button
          ...
          className="text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 disabled:opacity-50 rounded-md px-3 py-1.5"
          title="Re-import past interviews from ElevenLabs in case the tenant counter looks low."
        >
          {backfilling ? "Refreshing…" : "↻ Refresh from ElevenLabs"}
        </button>
      </div>
      {backfillMsg && (
        <p className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-md px-3 py-2 mb-3">
          {backfillMsg}
        </p>
      )}
